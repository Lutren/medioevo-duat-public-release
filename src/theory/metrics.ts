...
import { CHI_STAR, type CosmologyState, type SimulationMetrics, type SimulationParams } from "../simulation/types";
...
export class MetricsTracker {
  ...
  update(
    ...
    const centroidShift = Math.hypot(base.centroidX - this.previousCentroid.x, base.centroidY - this.previousCentroid.y);
    ...
    const osirisScore = clamp01(0.45 * liveness + 0.22 * persistenceScore + 0.2 * base.balance + 0.13 * (1 - Math.min(1, base.activity / 0.15)));
...
export function computeBaseMetrics(
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      const id = ix(x, y, width, height);
      const value = psi[id];
      sum += value;
      sum2 += value * value;
      activity += Math.abs(value - previous[id]);
      edge += Math.abs(value - psi[ix(x + 1, y, width, height)]) + Math.abs(value - psi[ix(x, y + 1, width, height)]);
      gx += x * value;
      gy += y * value;
      gSum += gravity[id];
      lSum += light[id];
...
export function computeRestrictionMatrixG(psi: Float32Array, width: number, height: number): Float32Array {
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      const center = psi[id];
      const neighborMean =
        (psi[ix(x - 1, y, width, height)] +
          psi[ix(x + 1, y, width, height)] +
          psi[ix(x, y - 1, width, height)] +
          psi[ix(x, y + 1, width, height)]) /
        4;
...
export function computeFisherL(psi: Float32Array, width: number, height: number): Float32Array {
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      const id = ix(x, y, width, height);
      const dx = (psi[ix(x + 1, y, width, height)] - psi[ix(x - 1, y, width, height)]) / 2;
      const dy = (psi[ix(x, y + 1, width, height)] - psi[ix(x, y - 1, width, height)]) / 2;
